...
// Fetch earnings based on the selected period
...
switch ($filter) {
    ...
    case 'year':
        ...
        break;
    case 'ytd':
        $dateQuery = "YEAR(transaction_date) = YEAR(CURDATE()) AND transaction_date <= CURDATE()";
        break;
}
...
<main>
    ...
    <div class="earnings-filter">
        ...
        <form action="earnings.php" method="get">
            <select name="filter" id="filter" onchange="this.form.submit()">
                ...
                <option value="ytd" <?php if ($filter == 'ytd') echo 'selected'; ?>>Year to Date</option>
            </select>
        </form>
    </div>
    ...
    <div style="display: flex; justify-content: space-around; text-align: center;">
        <div style="background-color: #4CAF50; color: white; padding: 20px; border-radius: 10px; width: 30%;">
            <p>Total Purchases</p>
            <h3>$<?php echo number_format($purchases, 2); ?></h3>
        </div>
        ...
        <div style="background-color: #FFC107; color: white; padding: 20px; border-radius: 10px; width: 30%;">
            <p>Net Earnings</p>
            <h3>$<?php echo number_format($earnings, 2); ?></h3>
        </div>
    </div>
</main>
